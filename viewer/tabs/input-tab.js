    ...
    <div class="report-section" id="section-input">
      ...
      <div class="file-controls">
        <label class="btn-secondary file-label hide-on-print">
          <input type="file" id="accdb-file-input" accept=".accdb,.ACCDB" style="display:none">
          📂 Load .ACCDB File
        </label>
      ...
      <div class="hide-on-print picker-block">
        ...
        ${pipeGroups.length 
          ? `<div class="table-scroll"><table class="data-table pipe-table">
              <thead><tr><th style="width:40px">Sel</th><th>OD (mm)</th><th>Insul (mm)</th><th>T1 (°C)</th><th>P1 (bar)</th><th>Density (kg/m³)</th><th>Material</th></tr></thead>
              <tbody>
                ${pipeGroups.map(g => `<tr>
                  ...
                  <td class="mono">${g.T1 ? fmt(g.T1, 0) : '—'}</td><td class="mono">${g.P1 ? fmt(g.P1, 2) : '—'}</td>
                  <td class="mono">${g.density ? fmt(g.density, 0) : '—'}</td><td>${g.material}</td>
                </tr>`).join('')}
              </tbody>
            </table></div>`
      ...
      <div id="prop-display-wrap" style="${state.inputToggles.props.length ? '' : 'display:none'}">
        ...
        <table class="data-table pipe-table" id="prop-display-table">
          <thead><tr><th>OD (mm)</th><th>Insul (mm)</th><th>T1 (°C)</th><th>P1 (bar)</th><th>Density (kg/m³)</th><th>Material</th></tr></thead>
      ...
      <div id="pinned-loads-wrap" style="${_pinnedLoads.length ? '' : 'display:none'}">
        ...
        <table class="data-table pinned-table" id="pinned-loads-table">
          <thead>
            <tr><th>Node</th><th>Fx (N)</th><th>Fy (N)</th><th>Fz (N)</th><th>Mx (N·m)</th><th>My (N·m)</th><th>Mz (N·m)</th><th></th></tr>
          </thead>
  ...
  select?.addEventListener('change', () => {
    ...
    preview.innerHTML = `
      <table class="data-table preview-table">
        <thead><tr><th>Node</th><th>Fx (N)</th><th>Fy (N)</th><th>Fz (N)</th><th>Mx (N·m)</th><th>My (N·m)</th><th>Mz (N·m)</th></tr></thead>
        <tbody><tr>
          <td class="mono">${f.node}</td>
          <td class="mono">${fmt(f.fx,0)}</td><td class="mono">${fmt(f.fy,0)}</td>
          <td class="mono">${fmt(f.fz,0)}</td><td class="mono">${fmt(f.mx,0)}</td>
